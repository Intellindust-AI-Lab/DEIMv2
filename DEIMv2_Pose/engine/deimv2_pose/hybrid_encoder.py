...
class HybridEncoder(nn.Module):
    ...
    def load_pretrained_weights(self, pretrain_path):
        ...
        Args:
            pretrain_path (str): Path to the pretrained checkpoint file
        """
        ...
        try:
            ...
            # Filter keys with 'encoder.' prefix and remove the prefix
            filtered_state_dict = {}
            ...
            for key, value in state_dict.items():
                if key.startswith(encoder_prefix):
                    ...
                    # Keep keys that don't have encoder prefix (for compatibility)
                    filtered_state_dict[key] = value
            
            if not filtered_state_dict:
                print(f"No encoder weights found in {pretrain_path}")
                return
            
            # Load state dict with strict=False to allow partial loading
            missing_keys, unexpected_keys = self.load_state_dict(filtered_state_dict, strict=False)
            
            print(f"Successfully loaded pretrained weights from {pretrain_path}")
            ...
            print(f"Failed to load pretrained weights from {pretrain_path}: {e}")
    ...
    def _reset_parameters(self):
        ...
        if self.pretrain and self.pretrain_path is not None:
            self.load_pretrained_weights(self.pretrain_path)
