...
class CustomModel(fom.Model):
    ...
    def predict_all(self, images):
        ...
        converted_predictions = [
            self._convert_predictions(pred) for pred in predictions
        ]
    ...
    for j, sample in tqdm.tqdm(
        ...
        for i, detection in enumerate(sample["predictions0"].detections):
            ...
            if (
                ...
                and sample["predictions5"].detections[i]["confidence"]
...
def assign_iou_diff(predictions_view):
    for sample in predictions_view:
        ...
        bbox_0 = [
            detection.bounding_box for detection in sample["predictions0"].detections
        ]
        bbox_5 = [
            detection.bounding_box for detection in sample["predictions5"].detections
        ]
...
def main(args):
    try:
        if fo.dataset_exists(args.dataset_name):
            print(f"Deleting existing FiftyOne dataset '{args.dataset_name}'")
            fo.delete_dataset(args.dataset_name)

        if os.path.exists("saved_predictions_view") and os.path.exists(
            "saved_filtered_view"
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-d",
        type=str,
        default="evaluate-detections-tutorial",
        help="Name to register the COCO validation split with in FiftyOne.",
    )
